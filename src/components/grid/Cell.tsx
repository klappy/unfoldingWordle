...
export const Cell = ({ value, status, delay = 0 }: Props) => {
  ...
  const classes = classnames(
    // Mobile-first responsive sizing - optimized for small screens
    'relative w-11 h-11 sm:w-14 sm:h-14 md:w-16 md:h-16 flex items-center justify-center mx-0.5 my-0.5 text-base sm:text-xl md:text-2xl font-bold rounded-lg sm:rounded-xl transition-all duration-300 transform',
    {
      // Default state
      'bg-white/70 backdrop-blur-sm border border-slate-200/50 sm:border-2 text-slate-700': !status,
      'dark:bg-slate-800/70 dark:border-slate-600/50 dark:text-slate-200': !status,
      
      // With value but no status
      'border-slate-400/70 scale-105': value && !status,
      ...
      'cell-animation': status,
      'hover:scale-105 active:scale-95': !status && !value,
    }
  )
